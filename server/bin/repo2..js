...
//       assignedTanods: report.assignedTanods,
...
export const createReport = asyncHandler(async (req, res) => {
  try {
    ...
    // ✅ Format tanods for frontend (WITH NAMES)
    ...
    const formattedReport = {
      ...
      evidence: report.evidence
        ? `${baseUrl}/uploads/${path.basename(report.evidence)}`
        : null,
      assignedTanods: formattedTanods, // ✅ WITH NAMES
      createdAt: report.createdAt,
    };
...
export const getSingleReport = asyncHandler(async (req, res) => {
  try {
    const report = await Report.findById(req.params.id)
      .populate("respondedBy", "fullName")
      .populate("assignedTanods", "fullName");
    ...
    const formatted = {
      ...
      assignedTanods: report.assignedTanods?.map((t) => t.fullName) || [],
